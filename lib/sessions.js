...
function Session(key)
{
	...
	this.expire = function() 
	{
		...
		logging.debug("<SESSION> Session %s expired forcefully.", key);
	};
...
function get(request, response)
{
	...
	{
		delete _SESSIONMAP[key];
		logging.debug("<SESSION> Session %s expired naturally.", key);
		key = _newSessionKey();
		out = _SESSIONMAP[key] = new Session(key);
		Helpers.setCookie(response, _SESSIONID, key);
		logging.debug("<SESSION> Session %s created. Expires in %d milliseconds.", key, options.timeout);
	}
	// update timeout.
	else
	{
...
function expire(request, response)
{
	if (request.cookies[_SESSIONID])
	{
		...
		logging.debug("<SESSION> Session %s expired forcefully. Cookie cleared.", key);
	}
}
