...
function sendFile(response, path, type, encoding)
{
	if (!type)
		type = mimetype(path);
...
function sendAttachmentFile(response, path, filename)
{
	...
	fs.readFile(path, (encoding ? encoding : 'utf8'), function(err, data)
	{
		if (err)
		{
			...
			_attach(response, 200, data, filename, mimetype(filename));
	});
}
...
function sendAuthenticateBasic(response, realmName)
{
	...
	response.write("401 Not Authorized\nMust supply proper credentials."); // "Not Authorized"
	response.end();
}
...
function checkAuthorization(request, authFunction)
{
	...
	switch (authtype)
	{
		case 'Basic':
		{
			var content = util.fromBase64(authcontent);
			if ((i = content.indexOf(':')) < 0)
			{
				logging.debug("Received bad auth data.");
				return false;
			}
			var a = [content.substring(0, i), content.substring(i + 1, content.length)];
