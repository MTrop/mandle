 ...
 * @param handlerArray the array of simple handler objects.
 * @param defaultOptions (optional) the default options to apply to each handler, if unspecified.
 */
function RouterMapping(handlerArray, defaultOptions)
{
	// PRIVATE MEMBERS
	...
	{
		processHandlerEntry(UTIL.combine({}, defaultOptions, handlerArray[i]));
	}
	else
		processHandlerEntry(UTIL.combine({}, defaultOptions, handlerArray));
