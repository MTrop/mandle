 ...
 * @returns {Function}
 */
function createFileUploadHandler(tempdir, handler)
{
	// TODO: Finish.
}
...
if (!MIME)
	console.log("INFO: 'Mime' module not installed. Using internal type tester.");

if (!JADE)
	console.log("INFO: 'Jade' module not installed. The Jade handler is not available.");
else
	exports.createJadeHandler = createJadeHandler;

if (!MUSTACHE)
	console.log("INFO: 'Mustache' module not installed. The Mustache handler is not available.");
else
	exports.createMustacheHandler = createMustacheHandler;

if (!MARKDOWN)
	console.log("INFO: 'Markdown' module not installed. The Markdown handler is not available.");
else
